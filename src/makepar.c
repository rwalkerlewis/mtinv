...
int main( int ac, char **av )
{
	...
	void select_lf_and_hf( float dist, float dt, float lf, float hf, float *new_lf, float *new_hf );
...
/******************************************************************/
	...
	for( ista=0; ista<nsta; ista++ )
	{
		...
		select_lf_and_hf( grn[ix][0].rdist, my_dt, lf, hf, &my_lf, &my_hf );

		if(local_setup)
		{
			if( grn[ix][0].rdist < 40 )
			{
				my_lf = 0.020;
				my_hf = 0.150;
			}
			...
			if( grn[ix][0].rdist < 250 ) my_dt = 0.1;
			if( grn[ix][0].rdist < 180 ) my_dt = 0.08;
			/*
			if( grn[ix][0].rdist < 160 ) my_dt = 0.07;
			if( grn[ix][0].rdist < 130 ) my_dt = 0.06;
			if( grn[ix][0].rdist < 100 ) my_dt = 0.05;
			*/
			if( grn[ix][0].rdist <  35 ) my_dt = 0.06;
		}
	...
	fprintf( fp, "/bin/rm -f plot_T???.?sec_Z???.?km_.p??.jpg plot_T???.?sec_Z???.?km_.p??.pdf automt.txt *.sql\n" );
...
/*********************************
	...
	fprintf( fp, "# mtinv evid=%ld ts0=0 %s par=mtinv.par mtdegfree=${DEGFREE} fwd  ### FixISOZ=1.0\n", gmtstring );
	fprintf( fp, "\n" );
*************************************/
	...
	{
	 fprintf(fp,
" mtinv evid=%ld AutoAuth ts0=${ts0} par=mtinv.par %s mtdegfree=${DEGFREE} use_snr minsnr=%g shift ctol=%g maxshift=%g >> mtinv.out\n",
                evid, gmtstring, minsnr, ctol, maxshift );
	}
	else
	{
	  fprintf(fp,
" mtinv evid=%ld ts0=${ts0} par=mtinv.par %s mtdegfree=${DEGFREE} use_snr minsnr=%g shift ctol=%g maxshift=%g >> mtinv.out\n",
		evid, gmtstring, minsnr, ctol, maxshift );
	}
	fprintf( fp, "end\n" );
	fprintf( fp, "\n" );
...
/***********************************/
	...
	if( !irealtime )
	{
	  ...
	  {
		fprintf( fp, "#   eog results.${DEGFREE}.jpg   #### older linux systems; use Eye-of-Gnome\n" );
		fprintf( fp, "display results.${DEGFREE}.jpg   #### newer linux systems; now use display by ImageMagick X-Windows\n" ); 
	  }
...
*/
	...
	{            /*** lower the resolution JPEG to just 120 pixesl per inch and remove PS ***/
		fprintf( fp, "  psconvert -Z -Tj -E120 ${ps}\n" ); /*** GMT 5.x.x ***/
	}
	else
	{
	...
	if( !irealtime )
	{
	  if( strcmp( my_uname.sysname, "Darwin" ) == 0 )
		...
		fprintf( fp, "open plot_T???.?sec_Z???.?km_.p??.jpg\n" );
          ...
          {
		fprintf( fp, "#  eog plot_T???.?sec_Z???.?km_.p??.jpg     #### older linux systems; use Eye-of-Gnome\n" );
                fprintf( fp, "display plot_T???.?sec_Z???.?km_.p??.jpg    #### newer linux systems; now use display by ImageMagick X-Windows\n" ); 
...
/***********************************/
	...
	if( strcmp( my_uname.sysname, "Darwin" ) == 0 )
        ...
        {
                fprintf( fp, "#   eog plotz.jpg   #### older linux systems; use Eye-of-Gnome\n" );
                fprintf( fp, "display plotz.jpg   #### newer linux systems; now use display by ImageMagick X-Windows\n" ); 
...
/**********************************************/
	...
	if( strcmp( my_uname.sysname, "Darwin" ) == 0 )
        ...
        {
                fprintf( fp, "#   eog plotmech.jpg    #### older linux systems; use Eye-of-Gnome\n" );
                fprintf( fp, "display plotmech.jpg    #### newer linux systems; now use display by ImageMagick X-Windows\n" ); 
...
/****************************************************/
	...
	if(!irealtime)
	{
	  ...
	  fprintf( fp, "\n" );
          ...
          {
                fprintf( fp, "#   eog gmtmap.jpg    #### older linux systems; use Eye-of-Gnome\n" );
                fprintf( fp, "display gmtmap.jpg    #### newer linux systems; now use display by ImageMagick X-Windows\n" ); 
	...
	{
		if(igmt5)
			fprintf( fp, "mtbestfit gmt5 evid=%ld\n", evid );
		else
			fprintf( fp, "mtbestfit nogmt5 evid=%ld\n", evid );
	}
...
void Print_Usage()
{
	...
	fprintf( stderr, "\t\t maxsta=(integer) max_number_stations_invert [default 8]\n" );
	fprintf( stderr, "\t\t mindist=(float) min_distance_define_km [default 80 km]\n" );
	fprintf( stderr, "\t\t maxdist=(float) max_distance_define_km [default 800 km]\n" );
