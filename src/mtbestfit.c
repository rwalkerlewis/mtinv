...
/*** functional prototypes ***/
	...
	void make_run( bestFitMT *a, char *fitType, int igmt5, long evid );
...
/*** begin main(), open input file ***/
	...
	fprintf( stderr,
"%s: %s: %s: reads a file named automt.txt and autogenerates scripts to make plots for real-time version\n",
...
/*** determine which is better fit or vred ? ***/
	...
	if( force_best_vred )
	{
		make_run( &(a[i_best_vred]), "FORCE BEST VARIANCE-REDUCTION", igmt5, evid );
	...
	else
	{
	  ...
	  {
		make_run( &(a[i_best_fit]), "BEST FIT", igmt5, evid );
		createResultsWebpage( &(a[i_best_fit]) );
	  }
	  else
	  {
		make_run( &(a[i_best_vred]), "BEST VARIANCE-REDUCTION", igmt5, evid );
		createResultsWebpage( &(a[i_best_vred]) );
	  }
	}
