...
/*** if the file does not exist this will be the first place ***/
	...
	/*****************************************************************/
	if(	( cbuf4[0] == 0x3c && cbuf4[1] == 0x21 ) || ( cbuf4[0] == 0x45 && cbuf4[1] == 0x72 ) ||
		( cbuf4[0] == 0x3c && cbuf4[1] == 0x3F ) )
	{
		if(verbose)
		{
		  fprintf( stdout, "This is a HTML or XML file: byte1=(0x%x) byte2=(0x%x)\n", cbuf4[0], cbuf4[1] );
		  fprintf( stderr, "This is a HTML or XML file: byte1=(0x%x) byte2=(0x%x)\n", cbuf4[0], cbuf4[1] );
		  fflush(stdout);
        	  fflush(stderr);
		}
		return -1;
